...
**数组简写**——直接写配件 ID 数组，行为与完整写法相同（可拆卸更换），但不支持指定 `default_attached`：

```json
"builtin_attachments": {
  "scope": ["tacz:sight_1", "tacz:sight_2"]
}
```
...
| `default_attached` | 可选，列表中默认预装的配件 ID（也接受 `"default"` 简写）。若不指定，该槽位初始为空 |
...
- 当枪械的 `exclusive_attachments` 中定义了某配件时，该枪械+配件组合使用此处定义的值；未定义时使用配件自身的全局数据。
