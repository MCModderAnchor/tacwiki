...
| `over_heat_time`     | 过热后冷却开始前的等待延迟（ms）            |
...
每发射一发子弹，热量增加 `per_shot` 点。当热量达到 `max` 时，枪械进入过热状态。

过热后需经过 `over_heat_time` 毫秒，热量才开始冷却。冷却期间热量持续下降至 0 后，枪械才能恢复开火。因此实际锁定时间 = `over_heat_time` + 热量从 `max` 降至 0 的冷却时间。

停火 `cooling_delay` 毫秒后，热量开始冷却（未触发过热时）。冷却量为线性：

```
冷却量 = (经过毫秒 / 10000) × cooling_multiplier
```
